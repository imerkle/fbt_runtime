 ...
 */
var IntlPhonologicalRewrites = require('IntlPhonologicalRewrites');
...
"\uFF0E" + // Fullwidth full stop
']';
var ENDS_IN_PUNCT_REGEXP = new RegExp(PUNCT_CHAR_CLASS + '[)"\'' + // JavaScript doesn't support Unicode character
...
'\\s' + ']*$');
var _rules = [];
var _lastLocale = null;

var _rewrites = IntlPhonologicalRewrites.get(IntlViewerContext.locale);
...
function _getRules() {
  ...
  if (!_rules.length) {
    ...
    for (var pattern in _rewrites.patterns) {
      var replacement = _rewrites.patterns[pattern]; // "Metaclasses" are shorthand for larger character classes. For example,
      // _C may refer to consonants and _V to vowels for a locale.

      for (var metaclass in _rewrites.meta) {
        var metaclassRegexp = new RegExp(metaclass.slice(1, -1), 'g');
        var characterClass = _rewrites.meta[metaclass];
...
function applyPhonologicalRules(text) {
  ...
  for (var i = 0; i < rules.length; i++) {
    var _rules$i = _slicedToArray(rules[i], 2),
        regexp = _rules$i[0],
